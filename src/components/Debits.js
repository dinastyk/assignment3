...
const Debits = (props) => {
  ...
  // api
  useEffect(() => {fetch('https://johnnylaicode.github.io/api/credits.json')
  .then(response => response.json())
  .then(data => {
...
const handleSubmit = (e) => {
  e.preventDefault();
  const amount = newDebit.amount.trim() === '' ? '0' : newDebit.amount; //empty string to 0
